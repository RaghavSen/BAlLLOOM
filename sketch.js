...
function setup() {
  ...
  balloon=createSprite(250,65);
  balloon.addAnimation("hotAirBalloon",balloonImage1);
  balloon.scale=0.5;

  var balloonHeight=database.ref('balloon/height');
  balloonHeight.on("value",readHeight,updateHeight,console.log("error"));
  textSize(20); 
}
...
function draw() {
  ...
  if(keyDown(LEFT_ARROW)){
    updateHeight(-10,0);
    balloon.addAnimation("ballomIMG1",balloonImage2)
    //add the animation of balloon [use balloonImage2]
  }
  else if(keyDown(RIGHT_ARROW)){
    updateHeight(10,0);
    balloon.addAnimation("ballomIMG2",balloonImage2)
    //add the animation of balloon [use balloonImage2]
  }
  else if(keyDown(UP_ARROW)){
    updateHeight(0,-10);
    balloon.addAnimation("ballomIMG3",balloonImage2)
 //add the animation of balloon [use balloonImage2]
    balloon.scale=balloon.scale -0.005;
  }
  else if(keyDown(DOWN_ARROW)){
    updateHeight(0,+10);
    balloon.addAnimation("ballomIMG4",balloonImage2)
